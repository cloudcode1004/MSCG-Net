...
class ChannelFusion(nn.Module):
    def __init__(self, c, *args, **kwargs):
        ...
        self.w = nn.Parameter(torch.zeros(c) * -2.0)  

        self.smooth = nn.Conv2d(c, c, kernel_size=3, padding=1, groups=c, bias=False)
    ...
    def forward(self, x):  
        ...
        alpha = torch.sigmoid(self.w).view(1, -1, 1, 1)
        out = alpha * p2 + p3
        return out

class P2_P3(nn.Module):
    def __init__(self, c1, *args, **kwargs):
        super().__init__()
        c2 = c1*2
        self.conv1 = Conv(c1, c2, k=1, s=1, p=0)
