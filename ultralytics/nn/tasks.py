    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if isinstance(f, list):
                # 多输入场景：按模块类型处理通道
                if m in {"Concat", "ChannelFusion"}:  # 多输入模块
                    c1 = sum(ch[x] for x in f) if m == "Concat" else ch[f[0]]  # Concat求和通道，其他取第一个输入通道
                else:  # 单输入模块误写为列表，取第一个元素
                    c1 = ch[f[0]]
            else:
                c1 = ch[f]  # 单输入直接取通道

            # 计算c2：args存在则取第一个元素，否则为None
            c2 = args[0] if args else None
            ####
